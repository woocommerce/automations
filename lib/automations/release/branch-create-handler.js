...
const branchHandler = async ( context, octokit, config ) => {
	...
	const matchingPullRequest = duplicateChecker( context, octokit, prTitle );
	...
	if ( isPatchRelease( releaseVersion ) ) {
		...
		pullRequestTemplate = getTemplate(
			TEMPLATES.patchReleasePullRequest,
			context,
			octokit
		);
		initialChecklistTemplate = getTemplate(
	...
	// get changelog
	const hasMilestone = hasMilestone( releaseVersion, context, octokit );
	try {
		if ( ! hasMilestone ) {
			throw new Error(
		...
		const changelogItems = getChangelogItems(
			context,
			octokit,
			releaseVersion,
			config
		);
		changelog = getChangelog( changelogItems, config );
